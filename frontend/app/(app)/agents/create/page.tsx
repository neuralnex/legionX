...
import { ChevronRight, ChevronLeft, Upload, Plus, Minus, Loader2, CheckCircle2, AlertCircle, X } from "lucide-react"
...
const agentSchema = z.object({
  ...
  minMemory: z.string().refine((val) => !isNaN(Number.parseInt(val)) && Number.parseInt(val) > 0, {
    message: "Memory must be a positive number",
  }),
  minStorage: z.string().refine((val) => !isNaN(Number.parseInt(val)) && Number.parseInt(val) > 0, {
...
type FormStep = "details" | "capabilities" | "technical" | "preview" | "submitting" | "success" | "error"
...
export default function CreateAgentPage() {
  ...
  const addCapability = () => {
    if (newCapability.trim() && !watchedValues.capabilities.includes(newCapability.trim())) {
      setValue("capabilities", [...watchedValues.capabilities, newCapability.trim()])
      setNewCapability("")
    }
  }
  ...
  const addDependency = () => {
    if (
      newDependency.trim() &&
      (!watchedValues.dependencies || !watchedValues.dependencies.includes(newDependency.trim()))
    ) {
      setValue("dependencies", [...(watchedValues.dependencies || []), newDependency.trim()])
      setNewDependency("")
    }
  }
      ...
      <div className="container mx-auto px-4 max-w-4xl">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          <div className="flex justify-between items-center mb-8">
            ...
            {currentStep !== "submitting" && currentStep !== "success" && currentStep !== "error" && (
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
              ...
              >
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs text-gray-500">{watchedValues.description.length}/500 characters</p>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-start space-x-4">
                      ...
                      <div className="flex-grow">
                        ...
                        <p className="mt-1 text-xs text-gray-500">Recommended: 800x800px. Max size: 5MB. PNG or JPG.</p>
                      </div>
                    </div>
                  </div>
              ...
              >
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    />
                    {errors.modelType && <p className="mt-1 text-sm text-red-400">{errors.modelType.message}</p>}
                  </div>
                  ...
                  <div>
                    ...
                    {errors.capabilities && <p className="mt-1 text-sm text-red-400">{errors.capabilities.message}</p>}
                    <p className="mt-1 text-xs text-gray-500">
                      Add capabilities like "Code Generation", "Data Analysis", etc.
                    </p>
                  </div>
                </div>
            ...
            {currentStep === "technical" && (
              <motion.div
                ...
                className="bg-gray-900/50 border border-gray-800 rounded-xl p-6"
              >
                <h2 className="text-xl font-semibold mb-6">Technical Requirements</h2>
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    />
                    {errors.version && <p className="mt-1 text-sm text-red-400">{errors.version.message}</p>}
                  </div>

                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label htmlFor="minMemory" className="block text-sm font-medium text-gray-300 mb-1">
                        Minimum Memory (GB) <span className="text-red-500">*</span>
                      </label>
                      <input
                        id="minMemory"
                        {...register("minMemory")}
                        ...
                        placeholder="e.g. 4"
                      />
                      {errors.minMemory && <p className="mt-1 text-sm text-red-400">{errors.minMemory.message}</p>}
                    </div>
                    <div>
                      <label htmlFor="minStorage" className="block text-sm font-medium text-gray-300 mb-1">
                        Minimum Storage (GB) <span className="text-red-500">*</span>
                      </label>
                      <input
                        id="minStorage"
                        {...register("minStorage")}
                        ...
                        placeholder="e.g. 1"
                      />
                      {errors.minStorage && <p className="mt-1 text-sm text-red-400">{errors.minStorage.message}</p>}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs text-gray-500">Add dependencies like "Node.js", "Python 3.8+", etc.</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div>
                    <div className="bg-gray-800 border border-gray-700 rounded-xl overflow-hidden">
                      ...
                      <div className="p-4">
                        <h3 className="font-semibold text-lg">{watchedValues.title || "Agent Title"}</h3>
                        <p className="text-gray-400 text-sm mb-3">Created by You</p>
                        <div className="flex justify-between items-center">
                          <span className="font-medium text-purple-400">
                            {watchedValues.price ? `${watchedValues.price} ETH` : "0.00 ETH"}
                          </span>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <p className="text-white">{watchedValues.description || "No description provided."}</p>
                    </div>

                    <div>
                      <h3 className="text-sm font-medium text-gray-400">Capabilities</h3>
                      <div className="flex flex-wrap gap-2 mt-1">
                        {watchedValues.capabilities.length > 0 ? (
                          watchedValues.capabilities.map((capability) => (
                            <span key={capability} className="bg-gray-800 text-gray-300 px-3 py-1 rounded-full text-sm">
                              {capability}
                            </span>
                          ))
                        ) : (
                          <p className="text-gray-500">No capabilities added.</p>
                        )}
                      </div>
                    </div>

                    <div>
                      <h3 className="text-sm font-medium text-gray-400">Technical Details</h3>
                      <div className="grid grid-cols-2 gap-4 mt-1">
                        <div>
                          <p className="text-xs text-gray-500">Model Type</p>
                          <p className="text-white">{watchedValues.modelType || "Not specified"}</p>
                        </div>
                        <div>
                          <p className="text-xs text-gray-500">Version</p>
                          <p className="text-white">{watchedValues.version || "Not specified"}</p>
                        </div>
                        <div>
                          <p className="text-xs text-gray-500">Min Memory</p>
                          <p className="text-white">{watchedValues.minMemory || "0"} GB</p>
                        </div>
                        <div>
                          <p className="text-xs text-gray-500">Min Storage</p>
                          <p className="text-white">{watchedValues.minStorage || "0"} GB</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2 mt-1">
                        {watchedValues.dependencies && watchedValues.dependencies.length > 0 ? (
                          watchedValues.dependencies.map((dependency) => (
                            <span key={dependency} className="bg-gray-800 text-gray-300 px-3 py-1 rounded-full text-sm">
                              {dependency}
                            </span>
                          ))
                        ) : (
                          <p className="text-gray-500">No dependencies added.</p>
                        )}
                      </div>
                    </div>
                    ...
                    {watchedValues.isPremium && (
                      <div className="bg-purple-900/30 border border-purple-800 rounded-lg p-3">
                        <p className="text-purple-400 text-sm">
                          This agent will be listed as premium and receive featured placement.
                        </p>
                      </div>
                    )}
                  </div>
            ...
            {currentStep === "submitting" && (
              <div className="bg-gray-900/50 border border-gray-800 rounded-xl p-8 text-center">
                ...
                <h3 className="text-xl font-semibold mb-2">Creating Your Agent</h3>
            ...
            {currentStep === "success" && (
              <div className="bg-gray-900/50 border border-gray-800 rounded-xl p-8 text-center">
                ...
                <p className="text-gray-400 mb-6">
                  Your AI agent has been created and is now available in the marketplace.
                </p>
                <motion.button
            ...
            {currentStep === "error" && (
              <div className="bg-gray-900/50 border border-gray-800 rounded-xl p-8 text-center">
                ...
                <p className="text-gray-400 mb-2">There was an error creating your agent. Please try again.</p>
