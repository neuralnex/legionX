...
export class LucidService {
  ...
  private async initializeLucid() {
    ...
    const oracleValidator: Script = {
                        ...
                        this.marketValidatorAddress = this.lucid.utils.validatorToAddress(marketValidator);
                        this.oracleValidatorAddress = this.lucid.utils.validatorToAddress(oracleValidator);
                } catch (error) {
  ...
  private async retry<T>(operation: () => Promise<T>): Promise<T> {
    ...
    throw lastError;
        ...
        getLucid(): Awaited<ReturnType<typeof Lucid.new>> {
    return this.lucid;
  }

  async getExchangeRate(currency: string): Promise<bigint> {
                const oracleUtxos = await this.lucid.provider.getUtxos(this.oracleValidatorAddress);
        ...
        async createListing(listing: Listing, seller: User): Promise<string> {
                try {
                        const metadata = JSON.stringify({
                                ...
                                full_price: listing.fullPrice?.toString() || undefined,
  ...
  async connectWallet(walletAddress: string): Promise<void> {
    try {
                        const utxos = await this.lucid.provider.getUtxos(walletAddress);
